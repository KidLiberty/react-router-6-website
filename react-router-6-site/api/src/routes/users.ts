...
// Router() is a lightweight version of Express() focused on handling routes
...
usersRouter.post('/login', async (req, res) => {
  ...
  if (user == null || !verifyPassword(body.password, user.salt, user.password)) {
...
usersRouter.post('/signup', async (req, res) => {
  ...
  const user = await db.user.create({
    data: { email: body.email, password: hash, salt: salt }
  })
